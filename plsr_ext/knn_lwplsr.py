...
class KNN_LWPLSR(RegressorMixin):
    ...
    """
    def __init__(self, n_components=2, k_nearest=10, lambda_in_similarity=1, sim_metric='euclidean'):
    ...
    def predict(self, X):
        ...
        for test_sample_number in range(X.shape[0]):
            ...
            ids_k_nearest, selected_x_train, distance = find_K_nearest_samples(self.scaled_x_train, query_x_test, self.k_nearest, self.sim_metric)
            ...
            y_w = selected_y_train.T.dot(np.diag(similarity)) / similarity.sum()
            x_w = np.reshape(selected_x_train.T.dot(np.diag(similarity)) / similarity.sum(), (1, selected_x_train.shape[1]))
            centered_y = selected_y_train - y_w
            centered_x = selected_x_train - np.ones((selected_x_train.shape[0], 1)).dot(x_w)
            ...
            for component_number in range(self.n_components):
                w_a = np.reshape(centered_x.T.dot(similarity).dot(centered_y) / np.linalg.norm(centered_x.T.dot(similarity).dot(centered_y)), (selected_x_train.shape[1], 1))
                t_a = np.reshape(centered_x.dot(w_a), (selected_x_train.shape[0], 1))
                p_a = np.reshape(centered_x.T.dot(similarity).dot(t_a) / t_a.T.dot(similarity).dot(t_a), (selected_x_train.shape[1], 1))
